...
class MLService:
    ...
    def load_model(self):
        try:
            ...
            print(f"[ML] Risk model load skipped/failed ({e}). Attempting self-healing retraining...")
            try:
                ...
                print(f"[ML] Self-healing retraining skipped: {train_err}")
    ...
    def predict_risk(self, age: int, gender_id: int, gravity_id: int, major_head_id: int, station_id: int) -> float:
        ...
        # Fallback heuristic logic if model is not trained/loaded or dependencies are missing
        score = 0.3
        if gravity_id == 1:
            score += 0.4
        if age < 30:

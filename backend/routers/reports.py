...
def generate_reportlab_pdf(case_data: dict, accused: list, victims: list, sections: list) -> bytes:
    ...
    story.append(Paragraph(f"PROJECT SENTINEL v2 — CONFIDENTIAL — {datetime.now().strftime('%d %b %Y')}", body_style))
...
async def generate_case_report(case_id: int):
    ...
    <body>
      <div class="header">
        ...
        <p>PROJECT SENTINEL v2 — CONFIDENTIAL INTEL REPORT</p>
      </div>
    ...
    return Response(
        ...
        headers={"Content-Disposition": f"attachment; filename=Sentinel_Report_{case['CrimeNo']}.txt"}
    )
...
async def generate_district_report(district_name: str):
    ...
    <body>
      <div class="header">
        <h1>PROJECT SENTINEL v2 — DISTRICT INTELLIGENCE AGGREGATION</h1>
      ...
      <footer style="margin-top: 40px; font-size: 9px; color: #888; text-align: center;">
        Generated on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} · Project Sentinel v2
      </footer>
    </body>
    </html>
    """
